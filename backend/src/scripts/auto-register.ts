...
async function run(eventId: string) {
  ...
  try {
    ...
    console.error(
      `❌ Event not found on Blockchain. Make sure Event ID is correct and event is initialized on-chain.`,
    );
    process.exit(1);
  }
  ...
  const statusValue = eventAccount.status.initialized
    ? 0
    : eventAccount.status.active
      ? 1
      : eventAccount.status.completed
        ? 2
        : eventAccount.status.settled
          ? 3
          : -1;
  ...
  if (statusValue !== 0) {
    ...
    console.error(
      `   Registration is only allowed when the event is in "Initialized" (pending) status.`,
    );
    console.error(
      `   ℹ️  Please create a NEW event from the Creator Dashboard and use that Event ID.`,
    );
    process.exit(1);
  }
  console.log(
    `✅ On-Chain Event status: ${statusLabel} (ready for registration)`,
  );
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (fs.existsSync(walletPath)) {
      ...
      console.log(
        `🔑 Using existing wallet: ${runnerKeypair.publicKey.toBase58()}`,
      );
    } else {
      runnerKeypair = Keypair.generate();
      fs.writeFileSync(
        walletPath,
        JSON.stringify(Array.from(runnerKeypair.secretKey)),
      );
      console.log(
        `🆕 Created new wallet: ${runnerKeypair.publicKey.toBase58()}`,
      );
    }
    ...
    if (solBalance < 50_000_000) {
      // Need at least 0.05 SOL for rent + gas
      console.log(
        `⚠️  Insufficient SOL (${(solBalance / 1e9).toFixed(4)} SOL). Requesting airdrop of 1 SOL...`,
      );
      try {
        ...
        console.log(
          `✅ Airdrop successful! Balance: ${(solBalance / 1e9).toFixed(4)} SOL`,
        );
      } catch (airdropErr: any) {
        console.error(`❌ Airdrop failed: ${airdropErr.message}`);
        console.error(
          `💡 Manually fund: solana airdrop 1 ${runnerKeypair.publicKey.toBase58()} --url devnet`,
        );
        failCount++;
        continue;
      }
  ...
  console.log(`\n===================================================`);
  console.log(
    `🎉 Auto-Register Complete! Success: ${successCount}/10, Failed: ${failCount}/10`,
  );
  console.log(`===================================================`);
  process.exit(0);
}
