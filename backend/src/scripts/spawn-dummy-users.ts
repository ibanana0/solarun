...
async function spawnDummies(eventId: string) {
  ...
  if (!["pending", "active"].includes(eventDb.status)) {
    console.error(
      `❌ Event status "${eventDb.status}" tidak mendukung registrasi.`,
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      `✅ Registration fee (on-chain): ${registrationFee} raw USDC\n`,
    );
  } catch (e) {
    console.warn(
      "⚠️  Tidak bisa fetch on-chain event. Melanjutkan dengan fee=0 (DB-only mode).",
    );
  }
  ...
  if (canSpawn <= 0) {
    console.log(
      "⚠️  Event sudah penuh atau DUMMY_COUNT = 0. Tidak ada yang di-spawn.",
    );
    process.exit(0);
  }
  ...
  for (let i = 0; i < canSpawn; i++) {
    ...
    if (sol < 50_000_000) {
      console.log(
        `   💧 SOL kurang (${(sol / 1e9).toFixed(4)}), requesting airdrop...`,
      );
      try {
    ...
    if (registrationFee > 0) {
      ...
      if (usdcBal < registrationFee) {
        console.log(
          `   💰 Mint Mock USDC (butuh ${registrationFee}, punya ${usdcBal})...`,
        );
        try {
    ...
    try {
      regTx = await (runnerProgram.methods as any)
        ...
        .rpc();
      console.log(
        `   ✅ On-chain registration OK. TX: ${regTx.slice(0, 20)}...`,
      );
    } catch (err: any) {
