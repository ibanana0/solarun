...
async function testBatching() {
  ...
  const { vaultAddress } = await initializeEventOnChain(
    ...
    startTime,
    endTime,
    new anchor.BN(3600), // 1 hour dispute lock
  );
  ...
  // NOTE: completeRaceOnChain now requires a treasury ATA.
  // In tests, pass the admin's own token account as treasury placeholder.
  // In production this should be the real treasury ATA from global state.
  const { PublicKey: PK } = await import("@solana/web3.js");
  const { getAssociatedTokenAddress } = await import("@solana/spl-token");
  const anchor2 = await import("@coral-xyz/anchor");
  const programId = process.env.SOLARUN_PROGRAM_ID!;
  const { getAdminKeypair: getKp, getConnection: getConn } =
    await import("./blockchain/transaction-signer");
  const conn2 = getConn();
  const adminKp2 = getKp();
  const [mintPda] = PK.findProgramAddressSync(
  ...
  await completeRaceOnChain(eventId, treasuryAta);
  ...
  console.log(
    `📦 Number of batches processed (from refund_logs): ${logs?.length || 0}`,
  );
  ...
  if ((logs?.length || 0) >= 2) {
    ...
    console.log(
      "❌ FAILURE: Expected at least 2 batches (Batch size for non-winners is 10).",
    );
  }
}
