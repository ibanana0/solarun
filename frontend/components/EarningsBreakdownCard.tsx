...
import { DollarSign, TrendingUp, TrendingDown, Zap, CheckCircle, Clock, AlertCircle, Loader2, ExternalLink } from 'lucide-react';
...
export default function EarningsBreakdownCard({
    ...
    const canClaim = status === 'completed' && stakeStatus === 'returned';
    ...
    return (
        <div className="border-2 border-primary p-lg space-y-lg">
            <div className="flex items-center justify-between">
                ...
                <span className={`font-label-caps text-label-caps px-sm py-xs border ${
                    isSlashed ? 'border-error bg-error/10 text-error' :
                    canClaim ? 'border-success bg-success/10 text-success' :
            ...
            <div className="space-y-sm">
                <p className="font-label-caps text-label-caps text-on-surface-variant uppercase">Revenue Sources</p>
                <div className="space-y-xs bg-surface-container p-md rounded-sm">
                    <div className="flex justify-between items-center">
                        ...
                        <span className="font-data-md text-success">+{totalDeposits.toFixed(2)} USDC</span>
                    </div>
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-sm">
                        ...
                        <span className="font-data-md text-orange-500">+{stakeAmount.toFixed(2)} USDC</span>
                    </div>
                    <div className="border-t border-outline-variant pt-xs flex justify-between font-label-caps">
                        ...
                        <span className="font-data-md text-primary">{grossAmount.toFixed(2)} USDC</span>
                    </div>
                </div>
            </div>
            ...
            {/* ── Fee Breakdown ────────────────────────────────────── */}
            <div className="space-y-sm">
                <p className="font-label-caps text-label-caps text-on-surface-variant uppercase">Protocol Fee Deduction</p>
                <div className="space-y-xs bg-surface-container p-md rounded-sm border border-error/30">
                    ...
                    <div className="flex justify-between items-center">
                        <span className="font-body-sm text-on-surface-variant">
                            Calculated from: {grossAmount.toFixed(2)} USDC × {protocolFeeBps} bps
                        </span>
                        <span className="font-data-md text-error">-{feeAmount.toFixed(2)} USDC</span>
                    </div>
                    {treasuryFeeCollected > 0 && (
                        <div className="flex justify-between items-center text-on-surface-variant text-body-xs">
                            <span>Fee Sent to Treasury</span>
                            <span className="text-error">{treasuryFeeCollected.toFixed(2)} USDC</span>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="bg-success/10 border-2 border-success p-md rounded-sm space-y-sm">
                <p className="font-label-caps text-label-caps text-success uppercase">Your Net Earnings</p>
                <div className="flex justify-between items-baseline">
                    <span className="font-headline-sm text-headline-sm text-success">Available to Claim</span>
                    <span className="font-display-md text-display-md text-success">{netAmount.toFixed(2)} USDC</span>
                </div>
                <p className="font-body-xs text-body-xs text-on-surface-variant">
                    This includes your original stake ({stakeAmount.toFixed(2)} USDC) minus the protocol fee.
                </p>
            </div>

            {/* ── Stake Status Indicator ───────────────────────── */}
            <div className="space-y-sm">
                <p className="font-label-caps text-label-caps text-on-surface-variant uppercase">Stake Status</p>
                <div className={`p-md border-l-4 space-y-xs ${
                    stakeStatus === 'pending' ? 'bg-on-surface-variant/5 border-l-on-surface-variant' :
                    stakeStatus === 'staked' ? 'bg-blue-900/10 border-l-blue-500' :
                    stakeStatus === 'returned' ? 'bg-success/10 border-l-success' :
                    'bg-error/10 border-l-error'
                }`}>
                    <div className="flex items-center gap-sm">
                        {stakeStatus === 'pending' && <Clock className="h-4 w-4 text-on-surface-variant" />}
                        {stakeStatus === 'staked' && <AlertCircle className="h-4 w-4 text-blue-500" />}
                        {stakeStatus === 'returned' && <CheckCircle className="h-4 w-4 text-success" />}
                        {stakeStatus === 'slashed' && <AlertCircle className="h-4 w-4 text-error" />}
                        <span className="font-body-sm font-bold uppercase">
                            {stakeStatus === 'pending' && 'Awaiting Deposit'}
                            {stakeStatus === 'staked' && 'Deposited & Locked'}
                    ...
                    <p className="font-body-xs text-on-surface-variant">
                        {stakeStatus === 'pending' && 'You must call stake_event on blockchain before starting.'}
                        {stakeStatus === 'staked' && 'Your stake is locked in the smart contract until event completion.'}
                        {stakeStatus === 'returned' && 'Your stake has been returned (minus protocol fee).'}
                        {stakeStatus === 'slashed' && 'Your stake was forfeited due to event cancellation/failure.'}
                    </p>
                </div>
            </div>
            ...
            <div className="border-2 border-outline-variant p-md space-y-xs">
                <p className="font-label-caps text-label-caps text-on-surface-variant uppercase mb-md">Summary</p>
                <div className="space-y-xs text-sm">
                    ...
                    <div className="flex justify-between py-xs border-t-2 border-primary pt-xs font-headline-sm">
                        ...
                        <span className="text-primary font-data-lg">{netAmount.toFixed(2)} USDC</span>
                    </div>
                </div>
            </div>
            ...
            {canClaim && (
                <div className="space-y-sm">
                    <button
                        ...
                        className="w-full bg-success text-background py-md font-label-caps text-label-caps hover:bg-success/90 transition-none uppercase disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-sm"
                    >
                        {claiming ? (
